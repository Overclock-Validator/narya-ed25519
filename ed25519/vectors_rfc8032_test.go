...
// These are all five plain Ed25519 known-answer vectors from RFC 8032,
// Section 7.1. Ed25519ctx, Ed25519ph, Ed448, and Ed448ph use different APIs
// and are intentionally outside this package's verification contract.
//
// Source: https://www.rfc-editor.org/rfc/rfc8032#section-7.1
func TestRFC8032Section7_1(t *testing.T) {
	for _, vector := range rfc8032Ed25519Vectors {
		vector := vector
		t.Run(vector.name, func(t *testing.T) {
			pubBytes := mustDecodeRFC8032Hex(t, vector.pub)
			message := mustDecodeRFC8032Hex(t, vector.message)
			signature := mustDecodeRFC8032Hex(t, vector.signature)
			if len(pubBytes) != 32 || len(signature) != 64 {
				t.Fatalf("invalid vector dimensions: pub=%d signature=%d", len(pubBytes), len(signature))
			}

			var pub [32]byte
			copy(pub[:], pubBytes)

			// Both versioned predicates must accept ordinary canonical RFC
			// signatures. check also covers the package, cache-hot, and explicit
			// precomputation paths.
			for _, profile := range []Profile{StdlibCompat, DalekStrict} {
				profile := profile
				profileName := "stdlib-compat"
				if profile == DalekStrict {
					profileName = "dalek-strict"
				}
				t.Run(profileName, func(t *testing.T) {
					withProfile(profile, func() {
						if !Verify(&pub, message, signature) {
							t.Fatal("Verify rejected an RFC 8032 signature")
						}
						check(t, &Cache{}, &pub, message, signature)
					})
				})
			}

			if !VerifyStrict(pubBytes, message, signature) {
				t.Fatal("VerifyStrict rejected an RFC 8032 signature")
			}
			cache := &Cache{}
			if !cache.VerifyStrict(&pub, message, signature) {
				t.Fatal("Cache.VerifyStrict rejected an RFC 8032 signature")
			}
			...
			ok := make([]bool, 1)
			if !VerifyBatchStrict(pubs, messages, signatures, ok) || !ok[0] {
				t.Fatal("VerifyBatchStrict rejected an RFC 8032 signature")
			}
			ok[0] = false
			if !cache.VerifyBatchStrict(pubs, messages, signatures, ok) || !ok[0] {
				t.Fatal("Cache.VerifyBatchStrict rejected an RFC 8032 signature")
			}
		})
	}
}

func mustDecodeRFC8032Hex(t *testing.T, encoded string) []byte {
	t.Helper()
	decoded, err := hex.DecodeString(encoded)
	if err != nil {
		t.Fatalf("invalid RFC 8032 test vector: %v", err)
	}
	return decoded
}

type rfc8032Ed25519Vector struct {
	name      string
	pub       string
	message   string
	signature string
...
var rfc8032Ed25519Vectors = []rfc8032Ed25519Vector{
	{
		name:      "TEST-1",
	...
	},
	{
		...
		message: "08b8b2b733424243760fe426a4b54908632110a66c2f6591eabd3345e3e4eb98fa6e264bf09efe12ee50f8f54e9f77b1" +
