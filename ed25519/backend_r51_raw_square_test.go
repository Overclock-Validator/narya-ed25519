...
func TestR51IFMARawSquareCompletePipelineDifferential(t *testing.T) {
	...
	for _, profile := range []Profile{DalekStrict, StdlibCompat} {
		t.Run(fmt.Sprintf("profile=%d/cctv", profile), func(t *testing.T) {
			assertR51IFMARawSquareVectors(t, r51CCTVVectors(t), profile)
		})
...
// BenchmarkR51IFMARawSquareCompletePipeline measures the exact dedicated-x8-
// square candidate at the public cold-verifier boundary. Both paths use the
