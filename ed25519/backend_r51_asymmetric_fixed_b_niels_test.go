...
func requireR51IFMABatchQX8AsymmetricFixedB10Pipeline(tb testing.TB) *r51IFMABatchQPipeline {
	tb.Helper()
	if !r51IFMAPipelineAvailable(r51IFMAX8) {
		tb.Skip("forced x8 r51 IFMA batch-Q pipeline unavailable")
	}
	pipeline, err := newR51IFMABatchQX8AsymmetricFixedB10PipelineWithFinalizer(r51IFMABatchQFinalizerLiteral)
	if err != nil {
		tb.Fatalf("new x8 asymmetric fixed-B10 pipeline: %v", err)
	}
	return pipeline
}

func TestR51IFMAAsymmetricFixedBNielsX8Differential(t *testing.T) {
	control := requireR51IFMABatchQX8CombPipeline(t)
	candidate := requireR51IFMABatchQX8AsymmetricFixedB10Pipeline(t)
...
func TestR51IFMAAsymmetricFixedBNielsX8ZeroAllocations(t *testing.T) {
	fixture := makeBatchFixture(t, 64, 1232)
	pipeline := requireR51IFMABatchQX8AsymmetricFixedB10Pipeline(t)
