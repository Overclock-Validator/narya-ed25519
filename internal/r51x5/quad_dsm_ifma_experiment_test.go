...
// quadCachePackedPointX4 converts [X,Y,T,Z] to [Y-X,Y+X,2dT,2Z]. The first
// helper performs the only subtraction normalization; one existing lane-wise
// field multiplication then applies all four coordinate constants together.
...
func quadNegateCachedPointX4(out *quadPackedCachedPointX4, positive *quadPackedCachedPointX4) {
	*out = *positive
	...
	// -(X,Y,Z,T) changes [Y-X,Y+X,2dT,2Z] to
	// [Y+X,Y-X,-2dT,2Z]. The composable table can contain redundant u52
	// representatives, so use the established biased-negation helper.
