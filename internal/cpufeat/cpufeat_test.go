...
func TestPackedPairX8ForAMDVersion(t *testing.T) {
	for _, test := range []struct {
		name   string
		ifma   bool
		family uint32
		...
		t.Run(test.name, func(t *testing.T) {
			if got := packedPairX8ForAMDVersion(test.ifma, test.family); got != test.want {
				t.Fatalf("packed-pair-x8=%v want=%v ifma=%v family=%#x", got, test.want, test.ifma, test.family)
			}
		})
	}
}
